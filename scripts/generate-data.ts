  ...
  const decisionLogs: AppDocument[] = [
    {
      ...
      body: [
        '## Decision',
        `Program management has agreed in principle to re-baseline ${troubled.name} rather than continue tracking it against its original plan or cancel it outright. The re-baseline will reset scope, schedule, and budget to reflect the work genuinely remaining, and will become the project's basis of record once ratified.`,
        '',
        '## Status',
        `Proposed ${asOf}; pending the re-baselined plan and steering ratification before it takes effect. Owned by ${troubled.owner} in the ${troubled.divisionName} division.`,
        '',
        '## Context',
    ...
    },
    {
      ...
      body: [
        ...
        '',
        '## Options considered',
        '- **Report from the Finance export.** Rejected: it omits records the PMO tracks, so the portfolio looks smaller than it is.',
        '- **Report from the PMO export.** Rejected: it carries duplicates, paints some statuses rosier than the underlying performance, and diverges from the financial system of record on budget.',
